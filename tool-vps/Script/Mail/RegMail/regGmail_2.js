...
async function regGmail_2({ browser, profileData, filePath, config }) {
    try {
        const mobileDevice = await MobileDevice.findOne({ GPMProfileId: profileData.id });
        let page = await navigation.newTab(browser, "");
        ...
        let isLogin = await page.evaluate(async () => {
            const btnLogin = document.querySelector('a[href*="https://accounts.google.com/SignOutOptions"],  a[href*="http://accounts.google.com/Logout"]');
            if (btnLogin) return true;
            return false;
        });
        ...
        if (!isLogin) {
            ...
            await page.evaluate(() => {
                try {
                    document.querySelector('a[href^="https://accounts.google.com/ServiceLogin"]').click();
                    return true;
                }
                catch (e){
                    return false;
            ...
            await page.evaluate(() => {
                try {
                    document.querySelector('li[data-value="en-US"]').click();
                    return true;
                }
                catch (e) {
                    console.error(e);
            ...
            await page.evaluate(() => {
                try {
                    document.querySelectorAll('button[jsaction]')[2].click();
                    return true;
                }
                catch (e){
                    return false;
            ...
            await page.evaluate(() => {
                try {
                    document.querySelector('ul').querySelector('li').click()
                    return true;
                }
                catch (e){
                    return false;
        ...
        console.error(e);
        console.log('Lỗi tại regGmail_2');
        return false;
    }
}
module.exports = regGmail_2;
