/*
    script reportVideo_1
    mô tả: report video hoặc channel
*/
...
// ]
async function reportVideo_1({ browser, filePath, config }) {
    try {
        let { listLinkChannel } = config;
        ...
        for (let i = 1; i <= countVideoBuff; i++) {
            try {
                ...
                let viewVideoData = await youtubeCommon.viewVideo({ browser, profileData, scriptId, page, filePath, config });
                ...
                if (viewVideoData.viewVideoEndScreen) {
                    await youtubeCommon.viewVideo({ browser, profileData, page, filePath, config: newConfig });
                }
                else {
                ...
                console.log('Buff view gặp lỗi tại reportVideo_1', filePath, e);
            }
        }
        ...
        console.error(e);
        console.log('Lỗi tại reportVideo_1');
        return false;
    }
}
module.exports = reportVideo_1;
