/*
    script viewYoutube_2
    mô tả: nuôi xem youtube vào trang chủ youtube, 
    random tìm trong các tab trên topbar, random 2-5 tab, mỗi tab ở lại lướt một chút
    sau đó random xem 1 video bất kỳ
*/
...
async function viewYoutube_2({browser, profileData, filePath, config}) {
    try {
        console.log(filePath);
        let { listKeyword } = config;
        if (!listKeyword) listKeyword = "";
        console.log('listKeyword tại viewYoutube_2: ', listKeyword);
        ...
        let maxTab = helper.randomInt(2, 5);
        // let page = await findActiveTab(page);
        for (let i = 1; i <= maxTab; i++) {
            process = await page.evaluate(() => {
                try {
                    var elements = document.querySelectorAll('.yt-chip-cloud-chip-renderer');
                    var randomIndex = Math.floor(Math.random() * elements.length); // lấy chỉ mục ngẫu nhiên
                    var randomElement = elements[randomIndex]; // truy cập phần tử có chỉ mục ngẫu nhiên
                    randomElement.click(); // click vào phần tử
                    return true;
                }
                catch (e) {
                    return false;
        ...
        let findVideo = await page.evaluate((watched) => {
            ...
            for (let i = 0; i < 5; i++) {
                let videoId;
                try {
                    let links = document.querySelectorAll('a[href*="/watch?v="]#video-title-link');
        ...
        if (!findVideo.process) {
            console.log('Không tìm thấy video nào viewYoutube_2');
            return false;
        }
        ...
        if (process) {
            // xem 1 video bất kì bên cột đề xuất, while đến khi chắc chắn tìm được 1 video (while đối đa 10 lần), xem liên tục 3-5 lần như vậy
            watched = (await helper.readFileAsync(filePath + "\\youtube-log\\watched-video-log.txt"))?.split('\n')?.filter(o => o !== '')?.map(o => o.split("|")[0]);
            let randomViewNextVideo = helper.randomInt(3, 5);
            for (let i = 1; i <= randomViewNextVideo; i++) {
        ...
        console.error(e);
        console.log('Lỗi tại viewYoutube_2');
        return false;
    }
}
module.exports = viewYoutube_2;
